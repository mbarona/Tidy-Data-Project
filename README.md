...
join activityLabels and activity_data to create new variable V2 which contains the activity names, used join function in the plyr package
# library(plyr)
...
extracting/subsetting data with measurement mean() and std(), using the subset and grepl functions
# subset_std <- subset(dataMelt,grepl("std()",dataMelt$variable))
# subset_mean <- subset(dataMelt,grepl("mean()",dataMelt$variable))

combining mean and std subsets to one dataset using rbind
